...
# High amount of missing values across columns
...
ggplot(yrbss, aes(x = weight)) +
  ...
  stat_function(
    ...
    color = "red",
    size = 2,
    args = list(mean = mean(yrbss$weight, na.rm = TRUE), sd = sd(yrbss$weight, na.rm = TRUE))
  ) +
...
weight_3plus_distribution <- yrbss %>% 
...
weight_3plus_distribution

CI_limits <- weight_3plus_distribution %>% 
  select(physical_3plus, weight_low, weight_high)

over3_upper <- weight_3plus_distribution[2, 9]
over3_lower <- weight_3plus_distribution[2, 8]
under3_upper <- weight_3plus_distribution[1, 9]
under3_lower <- weight_3plus_distribution[1, 8]
...
yrbss %>%
  ...
  geom_boxplot() +
  # add in confidence intervals if desired 
  # stat_summary(fund.data = mean_cl_boot, geom = "errorbar", color = "red") + 
...
```{r}
# H_0: Difference in mean = 0
# H_A: Difference in mean != 0
t.test(weight ~ physical_3plus, data = yrbss)
```
We can reject our null hypothesis with a p-value of 9e-08.
...
higher_lower <- as.numeric(IMDB_graph[1, 8])
lower_upper <- as.numeric(IMDB_graph[2, 9])


# Create graph
ggplot(IMDB_graph, aes(color = director)) +
  geom_errorbar(aes(x = reorder(director, mean_rating),
                    ymin = rating_low, 
                    ymax = rating_high),
                width = 0.2, 
                size = 1.8) +
  #Add grey ahding for CI overlap
  geom_rect(aes(
    ymin= higher_lower, 
    ymax = lower_upper, 
    xmin= 0, 
...
Null Hypothesis: Difference between mean rating = 0
Alternative Hypothesis: Difference between mean rating != 0 
...
```
```{r yield_curve_1_graph, echo=FALSE, out.width="100%"}

graph <- yield_curve %>% 
  drop_na(var) %>% 
  mutate(var = factor(var, levels = myvars)) %>% 
  ggplot() +
  geom_line(aes(x = date, y = price, color = var), size = 0.25) + 
...
```
```{r yield_curve_2_graph, echo=FALSE, out.width="100%"}
...
graph2 <- yield_curve %>% 
  ...
  filter(year >= "1999") %>% 
  ggplot() +
  geom_line(aes(x = maturity, y = price, color = year, group = month), size = 0.25) + 
...
```
```{r yield_curve_3_graph, echo=FALSE, out.width="100%"}
...
graph3 <- yield_curve %>% 
  ...
  ggplot() +
  geom_line(aes(x = date, y = price, group = var, color = var), size = 0.25) + 
...
graph4 <- yield_curve %>% 
  ...
  geom_rect(data = filter(recessions), inherit.aes=F, aes(xmin=From, xmax=To, ymin=-Inf, ymax=Inf),fill = "grey",alpha=0.5) +
  # create deviation and forecast (y=0) lines
  geom_line(aes(x = date, y = spread), color = "black", size = 0.25) +
  geom_line(aes(x = date, y = 0), color = "black", size = 0.25) + 
  # Create ribbon to highlight normal vs inverted yield curve
  geom_ribbon(aes(x = date, ymin = pmin(spread,0), ymax = 0, fill = "Normal"), alpha = .3) +
  geom_ribbon(aes(x = date, ymin = 0, ymax = pmax(spread,0), fill = "Inverted"), alpha = .3) +
  scale_fill_manual(values=c("blue","red"), name="Normal vs. Inverted") +
  # Create rug at bottom of the graphs
  geom_rug(aes(x = date, colour = ifelse(spread >= 0,">=0","<0")), sides = "b") +
  scale_colour_manual(values=c("red","blue"),name="Normal vs. Inverted", guide=FALSE) +
